...
const schema = object().shape({
    ...
    payload: string().when('action', {
        is: (v) => v !== 'backup',
        then: string().required('タスクのペイロードを入力してください。'),
        otherwise: string(),
    }),
    continueOnFailure: boolean(),
    timeOffset: number()
        .typeError('時間オフセットは0〜900の有効な数値である必要があります。')
        .required('時間オフセット値を入力してください。')
        .min(0, '時間オフセットは0秒以上である必要があります。')
        .max(900, '時間オフセットは900秒未満である必要があります。'),
});
                ...
                <Form css={tw`m-0`}>
                    ...
                    <div css={tw`flex`}>
                        ...
                        <div css={tw`flex-1 ml-6`}>
                            <Field
                                ...
                                label={'時間オフセット (秒)'}
                                description={
                                    '前のタスクが実行されてからこのタスクを実行するまでの待機時間。スケジュールの最初のタスクの場合は適用されません。'
                                }
                            />
                        </div>
                    </div>
                        ...
                        {values.action === 'command' ? (
                            ...
                            <div>
                                ...
                                <FormikFieldWrapper
                                    name={'payload'}
                                    description={
                                        '任意。このバックアップから除外するファイルやフォルダを指定します。デフォルトでは.pteroignoreファイルの内容が使用されます。バックアップ上限に達している場合、最も古いバックアップがローテーションされます。'
                                    }
                                >
                                    <FormikField as={Textarea} name={'payload'} rows={6} />
                                </FormikFieldWrapper>
                    ...
                    <div css={tw`mt-6 bg-neutral-700 border border-neutral-800 shadow-inner p-4 rounded`}>
                        <FormikSwitch
                            ...
                            description={'このタスクが失敗した場合、後続のタスクも実行されます。'}
                            label={'失敗時も続行'}
                        />
                    </div>
