...
@section('title')
    Egg &rarr; {{ $egg->name }} &rarr; 変数
@endsection

@section('content-header')
    <h1>{{ $egg->name }}<small>このEggの変数を管理します。</small></h1>
    <ol class="breadcrumb">
                ...
                <form action="{{ route('admin.nests.egg.variables.edit', ['egg' => $egg->id, 'variable' => $variable->id]) }}" method="POST">
                    <div class="box-body">
                        ...
                        <div class="row">
                            ...
                            <div class="col-xs-12">
                                <p class="text-muted small">この変数は起動コマンド内で <code>{{ $variable->env_variable }}</code> を使用してアクセスできます。</p>
                            </div>
                        </div>
                        <div class="form-group">
                            <label class="form-label">権限</label>
                            <select name="options[]" class="pOptions form-control" multiple>
                                <option value="user_viewable" {{ (! $variable->user_viewable) ?: 'selected' }}>ユーザーが閲覧可能</option>
                        ...
                        <div class="form-group">
                            ...
                            <p class="text-muted small">これらのルールは標準的な <a href="https://laravel.com/docs/11.x/validation#available-validation-rules" target="_blank">Laravelフレームワークのバリデーションルール</a>を使用して定義されています。</p>
                        </div>
                    </div>
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h4 class="modal-title">新しいEgg変数を作成</h4>
            </div>
            <form action="{{ route('admin.nests.egg.variables', $egg->id) }}" method="POST">
                <div class="modal-body">
                    ...
                    <div class="row">
                        ...
                        <div class="col-xs-12">
                            <p class="text-muted small">この変数は起動コマンド内で <code>@{{環境変数の値}}</code> と入力してアクセスできます。</p>
                        </div>
                    </div>
                    <div class="form-group">
                        <label class="control-label">権限</label>
                        <select name="options[]" class="pOptions form-control" multiple>
                            <option value="user_viewable">ユーザーが閲覧可能</option>
