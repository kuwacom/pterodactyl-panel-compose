...
@section('title')
    Nest &rarr; Egg: {{ $egg->name }} &rarr; インストールスクリプト
@endsection

@section('content-header')
    <h1>{{ $egg->name }}<small>このEggのインストールスクリプトを管理します。</small></h1>
    <ol class="breadcrumb">
                ...
                @if(! is_null($egg->copyFrom))
                    <div class="box-body">
                        <div class="callout callout-warning no-margin">
                            このサービスオプションは <a href="{{ route('admin.nests.egg.view', $egg->copyFrom->id) }}">{{ $egg->copyFrom->name }}</a> からインストールスクリプトとコンテナオプションをコピーしています。このスクリプトに加えた変更は、下のドロップダウンボックスから「なし」を選択しない限り適用されません。
                        </div>
                    </div>
                @endif
                ...
                <div class="box-body">
                    <div class="row">
                        <div class="form-group col-sm-4">
                            ...
                            <p class="text-muted small">選択した場合、上記のスクリプトは無視され、選択したオプションのスクリプトが代わりに使用されます。</p>
                        </div>
                        <div class="form-group col-sm-4">
                            ...
                            <p class="text-muted small">このスクリプトをサーバーで実行する際に使用するDockerコンテナ。</p>
                        </div>
                        <div class="form-group col-sm-4">
                            ...
                            <p class="text-muted small">このスクリプトで使用するエントリポイントコマンド。</p>
                        </div>
                    </div>
                    <div class="row">
