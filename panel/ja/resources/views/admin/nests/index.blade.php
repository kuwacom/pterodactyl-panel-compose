...
@section('content-header')
    <h1>Nest<small>このシステムで現在利用可能なすべてのNest。</small></h1>
    <ol class="breadcrumb">
        <li><a href="{{ route('admin.index') }}">管理者</a></li>
        <li class="active">Nest</li>
    </ol>
@endsection
...
<div class="row">
    <div class="col-xs-12">
        <div class="alert alert-danger">
            EggはPterodactyl Panelの強力な機能であり、極めて高い柔軟性と設定を可能にします。ただし、強力である反面、Eggを誤って変更するとサーバーが簡単に破損し、さらなる問題を引き起こす可能性があります。何をしているか完全に確信がない限り、<code>support@pterodactyl.io</code> から提供されているデフォルトのEggの編集は避けてください。
        </div>
    </div>
</div>
<div class="row">
    <div class="col-xs-12">
        <div class="box">
            <div class="box-header with-border">
                <h3 class="box-title">設定済みNest</h3>
                <div class="box-tools">
                    <a href="#" class="btn btn-sm btn-success" data-toggle="modal" data-target="#importServiceOptionModal" role="button"><i class="fa fa-upload"></i> Eggをインポート</a>
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h4 class="modal-title">Eggをインポート</h4>
            </div>
            <form action="{{ route('admin.nests.egg.import') }}" enctype="multipart/form-data" method="POST">
                <div class="modal-body">
                    <div class="form-group">
                        <label class="control-label" for="pImportFile">Eggファイル <span class="field-required"></span></label>
                        <div>
                            <input id="pImportFile" type="file" name="import_file" class="form-control" accept="application/json" />
                            <p class="small text-muted">インポートしたい新しいEggの <code>.json</code> ファイルを選択してください。</p>
                        </div>
                    </div>
                    <div class="form-group">
                        <label class="control-label" for="pImportToNest">関連付けるNest <span class="field-required"></span></label>
                        <div>
                            ...
                            <p class="small text-muted">ドロップダウンからこのEggを関連付けるNestを選択してください。新しいNestに関連付けたい場合は、続行する前にそのNestを作成する必要があります。</p>
                        </div>
                    </div>
                </div>
